...
802.11 無線LANでは、アクセス制御方式としてCSMA/CAが採用されている。802.11 無線LANの基本アクセス手順とされているDCF(Distributed Coordination Function)では、各局がチャネルの使用状況を検査して自律的にパケット(フレーム)の送信タイミングを決定するが、その時のアクセス制御プロトコルとしてCSMA/CAを使用している。
